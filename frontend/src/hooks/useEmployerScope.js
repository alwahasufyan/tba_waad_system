...
export const useEmployerScope = (selectedEmployerId = null) => {
  ...
  const isEmployerLocked = useMemo(() => 
    ['EMPLOYER_ADMIN', 'REVIEWER'].includes(userRole),
    [userRole]
  );
  ...
  const currentEmployerName = useMemo(() => {
    if (!effectiveEmployerId) return 'جميع أصحاب العمل';
    const employer = employers.find(e => e.id === effectiveEmployerId);
    return employer?.name ?? `صاحب العمل #${effectiveEmployerId}`;
