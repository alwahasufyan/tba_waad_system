...
const BenefitPolicyView = () => {
  ...
  const handleActivate = () => {
    openConfirmDialog(
      'activate',
      'تفعيل الوثيقة',
      'هل أنت متأكد من تفعيل هذه الوثيقة؟ سيتم إلغاء تفعيل أي وثيقة أخرى نشطة لنفس صاحب العمل.'
    );
  };
  ...
  const handleCancel = () => {
    openConfirmDialog(
      'cancel',
      'إلغاء الوثيقة',
      'هل أنت متأكد من إلغاء هذه الوثيقة نهائياً؟ هذا الإجراء لا يمكن التراجع عنه.'
    );
  };
      ...
      <>
        ...
        <MainCard>
          <Alert severity="error">
            {error.response?.data?.message || error.message || 'فشل تحميل بيانات الوثيقة'}
          </Alert>
          <Box sx={{ mt: 2 }}>
        ...
        <Grid item xs={12} md={6}>
          <MainCard title="معلومات التغطية">
            <DetailRow
              ...
              value={
                policy?.annualLimit 
                  ? `${Number(policy.annualLimit).toLocaleString('ar-SA')} د.ل` 
                  : 'غير محدد'
              }
              icon={MoneyIcon}
            />
