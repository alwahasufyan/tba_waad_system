...
const ClaimsFilters = ({
  ...
  const hasActiveFilters = 
    ...
    filters.minRequestedAmount !== '' ||
    filters.maxRequestedAmount !== '' ||
    filters.minApprovedAmount !== '' ||
    filters.maxApprovedAmount !== '';
  ...
  return (
    <Paper sx={{ p: 2, mb: 2 }}>
      ...
      <Grid container spacing={2}>
        ...
        {canSelectEmployer && (
          <Grid item xs={12} sm={6} md={3}>
            <FormControl fullWidth size="small">
              <InputLabel id="employer-filter-label">صاحب العمل</InputLabel>
              <Select
            ...
            InputProps={{
              ...
              endAdornment: filters.memberSearch && (
                <InputAdornment position="end">
                  <IconButton
                    size="small"
                    onClick={() => onFilterChange({ ...filters, memberSearch: '' })}
                  >
                    <ClearIcon fontSize="small" />
                  </IconButton>
                </InputAdornment>
        ...
        <Grid item xs={6} sm={3} md={canSelectEmployer ? 1.5 : 2}>
          <TextField
            fullWidth
            size="small"
            ...
            onChange={handleChange('minRequestedAmount')}
            placeholder="0"
            InputProps={{
              inputProps: { min: 0 }
            }}
        ...
        <Grid item xs={6} sm={3} md={canSelectEmployer ? 1.5 : 2}>
          <TextField
            fullWidth
            size="small"
            label="الحد الأقصى للمطلوب"
            type="number"
            value={filters.maxRequestedAmount}
            onChange={handleChange('maxRequestedAmount')}
            placeholder="∞"
            InputProps={{
              inputProps: { min: 0 }
            }}
        ...
        <Grid item xs={6} sm={3} md={canSelectEmployer ? 1.5 : 2}>
          <TextField
            fullWidth
            size="small"
            label="الحد الأدنى للمعتمد"
            type="number"
            value={filters.minApprovedAmount}
            onChange={handleChange('minApprovedAmount')}
            placeholder="0"
            InputProps={{
              inputProps: { min: 0 }
            }}
        ...
        <Grid item xs={6} sm={3} md={canSelectEmployer ? 1.5 : 2}>
          <TextField
            fullWidth
            size="small"
            label="الحد الأقصى للمعتمد"
            type="number"
            value={filters.maxApprovedAmount}
            onChange={handleChange('maxApprovedAmount')}
            placeholder="∞"
            InputProps={{
              inputProps: { min: 0 }
            }}
...
ClaimsFilters.propTypes = {
  filters: PropTypes.shape({
    ...
    minRequestedAmount: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
    maxRequestedAmount: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
    minApprovedAmount: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
    maxApprovedAmount: PropTypes.oneOfType([PropTypes.string, PropTypes.number])
  }).isRequired,
