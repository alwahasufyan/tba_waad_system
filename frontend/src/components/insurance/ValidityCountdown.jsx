...
const ValidityCountdown = ({
  ...
  const percentageUsed = validityDays > 0 
    ? Math.min(100, Math.max(0, ((validityDays - daysRemaining) / validityDays) * 100))
    : 100;
  ...
  return (
    <Box
      sx={{
        p: 2,
        borderRadius: 2,
        bgcolor: isExpired || isRejected ? 'error.lighter' 
               : isPending ? 'warning.lighter' 
               : isUsed ? 'info.lighter' 
               : isCritical ? 'error.lighter' 
               : isWarning ? 'warning.lighter' 
               : 'success.lighter',
        border: 1,
        borderColor: isExpired || isRejected ? 'error.light' 
                   : isPending ? 'warning.light' 
                   : isUsed ? 'info.light' 
                   : isCritical ? 'error.light' 
