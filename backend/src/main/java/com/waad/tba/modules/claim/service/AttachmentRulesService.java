...
public class AttachmentRulesService {
    ...
    public AttachmentValidationResult validateForSubmission(Claim claim, ClaimType claimType) {
        ...
        // Check for pre-approval requirement
        if (claimType.requiresPreApproval() && claim.getPreApproval() == null) {
            errors.add("Pre-approval is required for " + claimType.name() + " claims but none is linked");
        }
