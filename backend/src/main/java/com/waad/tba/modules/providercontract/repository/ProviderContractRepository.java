...
/**
 * Repository for Provider Contract entity.
 * 
 * Provides:
...
public interface ProviderContractRepository extends JpaRepository<ProviderContract, Long> {
     ...
     */
    @Query("SELECT c FROM ProviderContract c " +
     ...
     */
    @Query("SELECT COUNT(c) > 0 FROM ProviderContract c " +
           "WHERE c.provider.id = :providerId " +
           "AND c.id != :excludeId " +
           "AND c.active = true " +
     ...
     */
    @Query("SELECT COALESCE(SUM(c.totalValue), 0) FROM ProviderContract c " +
           "WHERE c.active = true " +
           "AND c.status = :status")
